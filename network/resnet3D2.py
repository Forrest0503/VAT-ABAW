...
class ResNet3D(nn.Module):
    ...
    #     self.tat_norm = nn.Sigmoid()
        ...
        # self.layer3_v = self._make_layer(block, 256, layers[2], shortcut_type, stride=2)
        # self.inplanes = self.inplanes_tmp
        # self.layer3_a = self._make_layer(block, 256, layers[2], shortcut_type, stride=2)
        #self.nl_3 = NONLocalBlock3D(in_channels=256)
        self.inplanes_tmp = self.inplanes
        self.layer4_v = self._make_layer(block, 512, layers[3], shortcut_type, stride=(1, 2, 2))
        #self.nl_4_v = NONLocalBlock3D(in_channels=512)
        self.inplanes = self.inplanes_tmp
        self.layer4_a = self._make_layer(block, 512, layers[3], shortcut_type, stride=(1, 2, 2))
        #self.nl_4_a = NONLocalBlock3D(in_channels=512)
        self.avgpool = nn.AdaptiveAvgPool3d(1)
        self.sp_pool = nn.AvgPool2d(4)
        self.fc_v = nn.Linear(512 * block.expansion, 1)
        self.fc_a = nn.Linear(512 * block.expansion, 1)
        self.fc_v_cls = nn.Linear(512* 4, 20)
        self.fc_a_cls = nn.Linear(512* 4, 20)
        self.fc_expr = nn.Linear(512 * 4 * 2, 7)
