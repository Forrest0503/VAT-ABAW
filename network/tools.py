...
class BNClassifier(nn.Module):
    ...
    def __init__(self, in_dim, class_num, initialization=True):
        ...
        self.classifier_va = nn.Linear(self.in_dim, self.class_num, bias=False)
        self.classifier_expr = nn.Linear(self.in_dim, 7, bias=False)
        if initialization :
            self.bn.apply(weights_init_kaiming)
        self.classifier_va.apply(weights_init_classifier)
        self.classifier_expr.apply(weights_init_classifier)

    def forward(self, x):
        feature = self.bn(x)
        cls_score_va = self.classifier_va(feature)
        cls_score_expr = self.classifier_va(feature)
        return feature, cls_score_va, cls_score_expr
