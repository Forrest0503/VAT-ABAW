...
class SpatioTemporalConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, bias=False, first_conv=False):
        ...
        if first_conv:
            ...
            intermed_channels = min(3200, int(math.floor((kernel_size[0] * kernel_size[1] * kernel_size[2] * in_channels * out_channels)/ \
...
class R2Plus1DNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.nl_3(x)
        x = self.conv4(x) # 256,14,14
        x = self.nl_4(x)
        x = self.conv5(x) # 512,7,7
        x = self.pool(x)
...
class R2Plus1DRegressor(nn.Module):
    ...
    def __init__(self, num_classes, layer_sizes, block_type=SpatioTemporalResBlock, pretrained=False):
        ...
        self.fc2_1 = nn.Linear(128, 2, bias=True)
        self.fc2_2 = nn.Linear(128, 7, bias=True)
