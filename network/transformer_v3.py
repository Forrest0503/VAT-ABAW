...
from network.tools import BNClassifier , BottleSoftmax
...
class Tail(nn.Module):
    def __init__(self, num_classes , num_frames, head=16):
        ...
        self.Qpr = nn.Conv2d(self.num_features, self.d_model, kernel_size=(4,4), stride=1, padding=0, bias=False)

        self.L1 = Block_head()
        self.L3 = Block_head()
        self.L2 = Block_head()
        
        self.classifier = BNClassifier(self.d_model+28*28, num_classes)
    ...
    def forward(self, x, b , t, diff):
        ...
        f = F.normalize(q, p=2, dim=1) # [b, 1024]
        diff = diff.view(f.shape[0], -1)
        f = torch.cat((f, diff), dim=1)
        if not self.training:
            f, y_va, y_expr = self.classifier(f)
            return y_va, y_expr
        f_, y_va, y_expr = self.classifier(f)
        return y_va, y_expr
...
class Semi_Transformer(nn.Module):
    ...
    def forward(self, x, avgface):
        ...
        for i in range(len(self.base)):
            x = self.base[i](x)
            if i <= 4:
                avgface = self.base[i](avgface)
            if i == 4:
                avgface = self.conv1x1(avgface)
                x1 = self.conv1x1(x)
                x1 = x1.view(b, t, x1.size(1), x1.size(2), x1.size(3))
                x1 = torch.mean(x1, 1)
                #print(x1.shape, avgface.shape)
                diff = x1 - avgface
        # x: (b,t,2048,7,4)
        return self.tail(x, b , t, diff)
